...
Yii::import('application.modules.profile.models.*');
     ...
     $criteria->with = array('profile');
      $criteria->together = false;
    //  echo '<prE>'; print_r(YumProfile::getProfileFields()); echo '</pre>';
      foreach(YumProfile::getProfileFields() as $column) {
   ...
   // echo '<pre>'; print_r($criteria); echo'</pre>';
    ...
    if (isset($_GET['s']))
			$criteria->addSearchCondition('t.username',$_GET['s'] , true, 'AND');
...
//				'message' => Yum::t($usernameRequirements['dontMatchMessage']));
	...
	public function relations()
	{
		...
		if($relations === false) {
			...
			$relations['goals'] = array(self::HAS_MANY, 'Goal', 'author_id');
