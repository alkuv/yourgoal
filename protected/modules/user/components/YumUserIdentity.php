...
class YumUserIdentity extends CUserIdentity {
	...
	public function authenticateFacebook() {
        ...
        //CREATED BY SHIV TO LOGIN WITH FB
        public function authenticateFacebookOauth($userId, $fbUId)
        {
                $this->id = $userId;
                $this->username = 'facebook';
                $this->facebook_id = $fbUId;
                //$this->facebook_user = $facebook->api('/me');
                $this->errorCode = self::ERROR_NONE;
        }
	...
	public function authenticate($without_password = false)
	{
		...
		return !$this->errorCode;
                ...
                public function loginByFb($email=null,$password=null)
        {             
                    $email    =  $this->username;
                    $password =  $this->password;
                    $record = YumUser::model()->findByAttributes(array('username'=>"$email"));
        ...
        public function loginByGoogle($email=null,$password=null)
        {             
                    $email    =  $this->username;
                    $password =  $this->password;
                    $record = YumUser::model()->findByAttributes(array('username'=>"$email"));
