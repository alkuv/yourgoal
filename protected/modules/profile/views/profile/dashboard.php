	...
	.editGoal{display:none}
	.dashgoals{text-decoration: none; color:#111}
     
        #draftGoal{display: none;}
	</style>
	<script>
	...
	<div class="profile-container">
		<div class="people-search-side clearfix">
			<div class="img-box">
				<?php if ($model->avatar!="" && $model->avatar!="gravatar") {?>
			...
			<div class="profile-info"> 
				...
				<div class="userObjc">
					...
					<?php echo  $cnt=Goal::model()->countByAttributes(array('author_id'=>$model->id,'status'=>"Active"));  ?>
					</span> active targets
					<span class="dot">.</span>
					<span class="completeObjs"><?php echo  $cnt=Goal::model()->countByAttributes(array('author_id'=>$model->id,'status'=>"Finished"));  ?></span> completed goals
				</div> 
			</div>    
			<div class="profile-control clearfix">
				<?php  $subslist=Subscrbe::model()->findAllByAttributes(array('author_id'=>Yii::app()->user->id,'following_id'=>$model->id));  ?>
				<?php if(!empty($subslist)) { ?> 
				<a href="<?php echo Yii::app()->createAbsoluteUrl("/goal/unfollow/follow/".$model->id);?>" class="yellow-btn">Unfollow</a>
				<?php } else { ?>
				<a href="<?php echo Yii::app()->createAbsoluteUrl("/goal/follow/follow/".$model->id);?>" class="yellow-btn">Follow</a>
				<?php } ?> 
				
				<a href="" class="link massage-link" style="display:none;">Messages</a><br>
			<!--	<a style="display:none;" href="#<?php //echo Yii::app()->createAbsoluteUrl("profile/profile/update");?>" class="link profile-link">Add news</a>-->
			</div> 
		</div>
		<div id="mainDragContainer">
			<div class="objectsBox clearfix">
				<div class="activeObjects" id="activeGoal">
					...
					<ul class=" activeObjects-list clear-ul sortable-list">
						<?php    $draftgoals=Goal::model()->findAllByAttributes(array('author_id'=>$model->id,'status'=>"Active"),array("order" => "id desc", "limit" => 15));  
						if(!empty ($draftgoals)) {
						foreach($draftgoals as $goals) {               
						?>
						<li class="activeObjects-item" id="<?php echo $goals->id;?>">
							<h4 class="item-objName">
								<input style="display:none" type="radio" onclick="complete(<?php echo $goals->id;?>)"name="complete" value=""/>
							...
							<div class="btmBox">
								...
								$record=Subscrbe::model()->findByAttributes(array('goal_id'=>$goals->id,'author_id'=>$model->id,'type'=>'subscribe')); 
								if(empty ($record)){
				...
				<div class="activeObjects" id="draftGoal">
					...
					<ul class="scrollbox2 activeObjects-list clear-ul sortable-list">
						<?php    $draftgoals=Goal::model()->findAllByAttributes(array('author_id'=>$model->id,'status'=>"Draft"),array("order" => "id desc", "limit" => 15));  
						if(!empty ($draftgoals)) {
						foreach($draftgoals as $goals) {               
						?>
						<li class="activeObjects-item" id="<?php echo $goals->id;?>">
				...
				<div class="activeObjects" id="completeGoal">
					...
					<ul class="scrollbox3 activeObjects-list clear-ul sortable-list">
						<?php    $draftgoals=Goal::model()->findAllByAttributes(array('author_id'=>$model->id,'status'=>"Finished"),array("order" => "id desc", "limit" => 15));  
						if(!empty ($draftgoals)) {
						foreach($draftgoals as $goals) {               
						?>
						<li class="activeObjects-item" id="<?php echo $goals->id;?>">
				...
				<div class="activeObjects" id="failedGoal">
					...
					<ul class="scrollbox4 activeObjects-list clear-ul sortable-list">
						<?php    $draftgoals=Goal::model()->findAllByAttributes(array('author_id'=>$model->id,'status'=>"Inactive"),array("order" => "id desc", "limit" => 15));  
						if(!empty ($draftgoals)) {
						foreach($draftgoals as $goals) {               
						?>
						<li class="activeObjects-item" id="<?php echo $goals->id;?>">
