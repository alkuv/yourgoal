...
//                        die("eddie");
                        if($_FILES['icon']['tmp_name']!='')
			{
				...
				{
					$filename=time(). '_' . str_replace(" ", "_", substr($model->name, 0, 20)) . '.' . $ext;
					
					$original='images/testimonial/original/' . $filename;
					$thumb='images/testimonial/thumb/' . $filename;
				
					if(move_uploaded_file($_FILES['icon']['tmp_name'], $original))
					{
						...
						$resizeObj1 -> resizeImage(143, 143, 'crop');
