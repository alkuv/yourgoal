...
class Goal extends CActiveRecord
{
       ...
       public function addComment($comment)
	{
		...
		$comment->goal_id=$this->id;
                ...
                //mail to goal admin start 
                  $connection = Yii::app()->db;
                  $login=Yii::app()->user->id;
                  if(!empty ($login)) {
                  $sql="select * from profile where user_id=$login";
                  $command = $connection->createCommand($sql);
                  $row = $command->queryRow();                  
                  $addedby=$row['firstname'];
                  // get goal admin detail
                  $goalrecord=Goal::model()->findByPk($this->id); 
                  ...
                  $connection = Yii::app()->db;
                   ...
                   $goalname=$this->name;
                  MailHelpers::createGoalcommenttoadmin($name,$adminemail,$addedby,$goalname);
                  }
                // mail to goal admin end
                
              
                  //send to all start 
                  //$all_subcriber = Subscrbe::model()->find(array('order'=>'id DESC', 'condition'=>'goal_id =:x AND status = :y', 'params'=>array(':x'=>$this->id,':y'=>'subscribe')));
                  $all_subcriber=Subscrbe::model()->findAll(array("condition"=>"goal_id =  '$this->id' AND type = 'subscribe'"));
                        ...
                        foreach($all_subcriber as $subcriber)
                            {
                               $sub_id=$subcriber->author_id;
                                $sql="select * from profile where user_id=$sub_id";
                                 ...
                                 MailHelpers::createGoalcommenttoall($goalname,$email,$addedby,$mainname);
                              // echo '<pre>';print_r($subscriber_record); echo '</pre>';
                             }
                      }
	...
	{
		parent::afterDelete();
		$comment=Comment::model()->findAll('goal_id='.$this->id);
                if(!empty ($comment)) {Comment::model()->deleteAll('goal_id='.$this->id);}
                $subscribe=Subscrbe::model()->findAll('goal_id='.$this->id);
                if(!empty ($subscribe))
                    {
                     Subscrbe::model()->deleteAll('goal_id='.$this->id);
                    }
